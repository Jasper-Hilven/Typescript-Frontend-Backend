/* tslint:disable */
module backend {
  export class CoreApi {

    constructor(
      private keyValidator: KeyValidator,
      private chipinProvider: ChipinProvider,
      private logger: commonend.Logger) {
    }

    GetChipmentUser(key, id) {
      this.logger.Info("getting chipment for a user");
      if (!this.keyValidator.IsValidChipinUser(id, key))
        return undefined;
      let chipin = this.chipinProvider.GetChipment(id);
      if (!chipin)
        return undefined;
      return chipin;
    };

    GetChipmentAuthor(key, id): commonend.ChipmentForAuthor {
      if (!this.keyValidator.IsValidChipinAuthor(id, key))
        return undefined;
      let chipment = this.chipinProvider.GetChipment(id);
      if (!chipment)
        return null;
      let userKey = this.keyValidator.GetUserKeyViaAuthor(id, key);
      return new commonend.ChipmentForAuthor(chipment, userKey, key, id);
    };

    CreateChipment(key, creatorId, info): commonend.ChipmentForAuthor {
      if (!this.keyValidator.IsValidCreateKey(creatorId, key)) {
        this.logger.Warn("Chipment creation aborted: invalid key");
        return null;
      }
      let chipmentId = this.keyValidator.CreateIdViaCreateId(creatorId);
      let userKey = this.keyValidator.CreateUserKey(chipmentId);
      let authorKey = this.keyValidator.CreateAuthorKey(chipmentId);
      let chipment = this.chipinProvider.GetChipment(chipmentId);
      if (chipment) {
        this.logger.Warn("Chipment creation aborted: Already exists");
        return null;
      }
      let newChipment = this.chipinProvider.CreateChipment(chipmentId, info);
      if (!newChipment) {
        this.logger.Warn("Chipment creation aborted: Provider could not create");
        return null;
      }
      return new commonend.ChipmentForAuthor(chipment, userKey, authorKey, chipmentId);
    };

    SetChipment(key, id, info) {
      if (!this.keyValidator.IsValidChipinAuthor(id, key))
        return false;
      let chipin = this.chipinProvider.GetChipment(id);
      if (!chipin)
        return false;
      return this.chipinProvider.SetChipment(id, info);
    };

    RemoveChipment(key: string, id: string): boolean {
      if (!this.keyValidator.IsValidChipinAuthor(id, key))
        return false;
      let chipin = this.chipinProvider.GetChipment(id);
      if (!chipin)
        return false;
      return this.chipinProvider.DeleteChipment(id);
    };

    CreateChipin(key: string, id: string, info): number {
      if (!this.keyValidator.IsValidChipinUser(id, key))
        return undefined;
      let chipment = this.chipinProvider.GetChipment(id);
      if (!chipment)
        return undefined;
      return this.chipinProvider.CreateAndAddChipin(id, info);
    };
    ChangeChipin(key: string, id: string, chipinid: number, info): boolean {
      if (!this.keyValidator.IsValidChipinUser(id, key))
        return false;
      let chipment = this.chipinProvider.GetChipment(id);
      if (!chipment)
        return false;
      return this.chipinProvider.SetChipinOfChipment(id, chipinid, info);
    };
    DeleteChipin(key: string, id: string, chipinid): boolean {
      if (!this.keyValidator.IsValidChipinUser(id, key))
      { return false; }
      let chipin = this.chipinProvider.GetChipment(id);
      if (!chipin)
      { return false; }
      return this.chipinProvider.DeleteChipinOfChipment(id, chipinid);
    };
  }
}
